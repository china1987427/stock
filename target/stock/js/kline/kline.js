...
$.getJSON('stockWorld/dynData', {
	...
	if (stockCode == "dapan") {
		...
		ymin = parseFloat(minli) - parseFloat(minli) * 0.005;
		ymax = parseFloat(maxhi) + parseFloat(maxhi) * 0.005;
	}
});
...
var klineTop = $("#mycanvas").offset().top;
x_scale = data.width / splitStep;// x轴刻度 50
y_scale = data.height / splitStep;// y轴刻度 25
leastSplit = (data.width - 2 * x_scale) / (number * 4);// 3.75
stepWidth = (data.width - 2 * x_scale) / len;// 15
var init = function(data) {
	...
	for (i = 0; i < kdata.length; i++) {
		...
		var scaleMark = (data.height - 4 * y_scale) / (data.yAxis[0].maxValue - data.yAxis[0].minValue);
		var y = 0;
		var h = scaleMark * (closingIndex - openIndex).toFixed(2);
		if (h > 0) {
			y = (scaleMark * (data.yAxis[0].maxValue - closingIndex) + (y_scale * 3)).toFixed(2);
		} else {
			y = (scaleMark * (data.yAxis[0].maxValue - openIndex) + (y_scale * 3)).toFixed(2);
		}
		var w = kwidth;
		var mx = x_scale + leastSplit * (2 + 4 * i);
		var my = (scaleMark * (data.yAxis[0].maxValue - highIndex) + (y_scale * 3)).toFixed(2);
		var lx = mx;
		var ly = (scaleMark * (data.yAxis[0].maxValue - lowIndex) + (y_scale * 3)).toFixed(2);
		stepXArr.push(x);
		if (h > 0) {
	...
	// 均线
	// 白色5，黄色10，紫色20，绿色30，蓝色60
	// [ "red", "white", "yellow", "purple", "green", "blue", "black" ]
	var indexArray = [];
	var color = "black";
	for ( var b = 1; b < 6; b++) {
		if (b == 1) {
			color = rectColor[6];
			ctx.fillStyle = color;
			indexArray = fiveindex.slice(0, number + 1);
			ctx.fillText("M5:" + fiveindex[0], x_scale + 150, y_scale - 8);
		} else if (b == 2) {
			color = rectColor[2];
			ctx.fillStyle = color;
			indexArray = tenindex.slice(0, number + 1);
			ctx.fillText("M10:" + tenindex[0], x_scale + 250, y_scale - 8);
		} else if (b == 3) {
			color = rectColor[3];
			ctx.fillStyle = color;
			indexArray = twentyindex.slice(0, number + 1);
			ctx.fillText("M20:" + twentyindex[0], x_scale + 350, y_scale - 8);
		} else if (b == 4) {
			color = rectColor[4];
			ctx.fillStyle = color;
			indexArray = thirtyindex.slice(0, number + 1);
			ctx.fillText("M30:" + thirtyindex[0], x_scale + 450, y_scale - 8);
		} else if (b == 5) {
			color = rectColor[5];
			ctx.fillStyle = color;
			indexArray = sixtyindex.slice(0, number + 1);
			ctx.fillText("M60:" + sixtyindex[0], x_scale + 550, y_scale - 8);
		}
		...
		for ( var a = 1; a <= 60; a++) {
			...
			if (a % 2 != 0) {
				...
				y0 = (scaleMark * (data.yAxis[0].maxValue - indexArray[a - 1]) + (y_scale * 3)).toFixed(2);
				y1 = (scaleMark * (data.yAxis[0].maxValue - indexArray[a]) + (y_scale * 3)).toFixed(2);
				y2 = (scaleMark * (data.yAxis[0].maxValue - indexArray[a + 1]) + (y_scale * 3)).toFixed(2);
				ctx.beginPath();
				ctx.moveTo(x0, y0);
...
var drawXAxis = function(xAxis) {
	...
	var height = data.height - y_scale;
	var width = data.width - x_scale;
	// 画x轴横线
	ctx.moveTo(width, height);
	ctx.lineTo(x_scale, height);
	// 加标签
	len = data.xAxis[0].xdata.length;
	stepWidth = (width - x_scale) / len;// 一个类型所占的宽度
	for ( var i = 1; i <= (number / 3); i++) {
		...
		if ((i - 1) == 0) {
			ctx.fillText(data.xAxis[0].xdata[0], x_scale - 8, height + 20);
		}
		ctx.fillText(data.xAxis[0].xdata[2 + (i - 1) * 3], parma, height + 20);
		ctx.globalCompositeOperation = "destination-over";
		ctx.moveTo(parma, height - 5);
		ctx.lineTo(parma, height);
	}
	ctx.stroke();
	// 加箭头
	drawArrow(width, height, false);
}
...
var drawYaxis = function(maxValue, step) {
	...
	var height = data.height - 3 * y_scale;
	// 加标签
	stepHeight = height / step;
	heightVal = height / maxValue;// 比例
	for ( var i = 10; i >= 0; i--) {
		var variable = ((10 - i) * ((data.height - 4 * y_scale) / 10)) + 3 * y_scale;
		ctx.font = "normal normal bold 12px 微软雅黑";
		// 字体居中
		ctx.fillText(((maxValue - ((maxValue - data.yAxis[0].minValue) / 10) * (10 - i))).toFixed(2), x_scale - 25,
				variable);
		ctx.globalCompositeOperation = "destination-over";
		ctx.moveTo(x_scale, variable);
		ctx.lineTo(data.width - x_scale, variable);
	}
	ctx.stroke();
	// 加箭头
...
var mouseMove = function(e) {
	...
	var x = e.clientX;
	var y = e.clientY;
	var top = parseFloat($("#canvasPanel").css("top").replace("px", ""));
	var left = parseFloat($("#canvasPanel").css("left").replace("px", ""));// 位置不能变动
	for ( var i = 0; i < number; i++) {
		if ((parseFloat(stepXArr[i])) + klineLeft <= x
				&& x <= (parseFloat(stepXArr[i]) + parseFloat(2 * leastSplit)) + klineLeft && 3 * y_scale <= y
				&& y <= data.width - y_scale) {
