...
var drawXAxis = function(xAxis) {
	...
	for (var i = 1; i <= (number/3); i++) {
		...
		if ((i - 1) == 0) {
			ctx.fillText(data.xAxis[0].xdata[0], x_scale-8, canvas.height
					- y_scale + 20);
		}
		ctx.fillText(data.xAxis[0].xdata[2 + (i - 1) * 3], parma, data.height
				- y_scale + 20);
...
var mouseMove = function(e) {
	...
	if (array[1] != undefined) {
		$("#show_num").show();
		// 显示数字
		$("#show_num").html('<div class="show_num"><span>日期：'
